...
const weatherOptions: { value: WeatherCondition; label: string; icon: 'sun' | 'cloud' | 'cloud-rain' | 'cloud-lightning' }[] = [
...
export default function NewRDOPage() {
  ...
  const [equipment, setEquipment] = useState<EquipmentEntry[]>([
    { id: '1', name: 'Betoneira 400L', horimeterStart: 1250, horimeterEnd: 1258 },
    { id: '2', name: 'Retroescavadeira', horimeterStart: 3420, horimeterEnd: 3428 },
  ])
  ...
  const updateEquipment = (id: string, field: 'horimeterStart' | 'horimeterEnd', value: number) => {
    setEquipment(prev =>
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 1: return weather !== null
      case 2: return workforce.length > 0
      case 3: return true
      case 4: return activities.trim().length > 0
      case 5: return signature !== null
      default: return true
    }
  }
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <header className="pt-safe sticky top-0 bg-background/95 backdrop-blur-sm z-10">
          <div className="px-6 py-4 flex items-center justify-between">
            ...
            <div className="text-center">
              <h1 className="text-lg font-bold text-foreground">RDO #{rdoNumber}</h1>
        ...
        <main className="flex-1 px-6 pb-32 overflow-y-auto">
                ...
                {location ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 p-4 bg-success/10 rounded-xl">
                      <BoxIcon name="check-circle" type="solid" size={24} className="text-success" />
                      <div>
                        <p className="font-medium text-foreground">Localização capturada</p>
                        <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-4 p-4 bg-warning/10 rounded-xl">
                    <BoxIcon name="current-location" size={24} className="text-warning animate-pulse" />
                    <div>
                      <p className="font-medium text-foreground">Obtendo localização...</p>
                      <p className="text-sm text-muted-foreground">Aguarde ou permita acesso ao GPS</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="ios-card">
                <h2 className="text-xl font-semibold mb-4">Condição do Tempo</h2>
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-4">
                    <BoxIcon 
                      name={weatherOptions.find(w => w.value === weather)?.icon || 'sun'} 
                      type="solid"
                      size={32} 
                  ...
                  <BoxIcon name="chevron-right" size={24} className="text-muted-foreground" />
                </button>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-xl bg-card border border-dashed border-border text-center">
                <p className="text-muted-foreground">
                  Total: <span className="font-bold text-foreground text-2xl">
          ...
          {currentStep === 3 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold mb-2">Horímetro de Equipamentos</h2>
              ...
              {equipment.map((eq) => (
                <div key={eq.id} className="ios-card">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Stepper
                        ...
                        onChange={(val) => updateEquipment(eq.id, 'horimeterStart', val)}
                        min={0}
                        max={99999}
                        step={1}
                      />
                    ...
                    <div>
                      <label className="text-sm text-muted-foreground mb-2 block">Final</label>
                      <Stepper
                        value={eq.horimeterEnd}
                        onChange={(val) => updateEquipment(eq.id, 'horimeterEnd', val)}
                  ...
                  <p className="text-sm text-muted-foreground mt-3 text-center">
                    Horas trabalhadas: <span className="font-bold text-foreground">
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <LargeTextarea
                ...
                showVoiceInput
                onVoiceResult={(text) => setActivities(prev => prev + (prev ? ' ' : '') + text)}
              />
              ...
              <LargeTextarea
                ...
                showVoiceInput
                onVoiceResult={(text) => setObservations(prev => prev + (prev ? ' ' : '') + text)}
              />
              ...
              <LargeTextarea
                ...
                showVoiceInput
                onVoiceResult={(text) => setIssues(prev => prev + (prev ? ' ' : '') + text)}
              />
            </div>
          )}
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <div className="ios-card">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{weatherOptions.find(w => w.value === weather)?.label}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{workforce.reduce((sum, w) => sum + w.quantity, 0)} pessoas</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Equipamentos</span>
                    <span className="font-medium">{equipment.length} unidades</span>
                  </div>
                </div>
              </div>
              ...
              <p className="text-sm text-muted-foreground text-center">
                Ao assinar, você confirma que as informações deste RDO são verdadeiras.
              </p>
            </div>
          )}
        </main>
          ...
          <div className="flex gap-4">
            ...
            {currentStep < steps.length - 1 ? (
              ...
              >
                {isSaving ? (
                  <>
                    <BoxIcon name="loader-circle" size={20} className="animate-spin" />
                    Salvando...
                  </>
                ) : (
                  <>
          ...
          <div className="space-y-3">
            {weatherOptions.map((option) => (
              <BottomSheetOption
                key={option.value}
                label={option.label}
                icon={<BoxIcon name={option.icon} type="solid" size={24} />}
