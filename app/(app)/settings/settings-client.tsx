"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { signOut } from "next-auth/react";
import { BoxIcon } from "@/components/ui/box-icon";
import { cn } from "@/lib/utils";
import { BottomSheet } from "@/components/ui/bottom-sheet";
import { NetworkStatusIndicator } from "@/components/ui/network-status";

interface SettingItem {
  icon?: string;
  label: string;
  value?: string;
  action?: () => void;
  danger?: boolean;
}

interface SettingSection {
  title: string;
  items: SettingItem[];
}

interface SettingsClientProps {
  user: {
    name: string | null;
    email: string | null;
    role: string | null;
  };
  projects: Array<{
    id: string;
    name: string;
  }>;
}

export default function SettingsClient({
  user,
  projects,
}: SettingsClientProps) {
  const router = useRouter();

  // Modais Básicos
  const [showLogoutSheet, setShowLogoutSheet] = useState(false);
  const [showClearCacheSheet, setShowClearCacheSheet] = useState(false);

  // Edição de Perfil
  const [showEditProfile, setShowEditProfile] = useState(false);
  const [editName, setEditName] = useState(user.name || "");
  const [editEmail, setEditEmail] = useState(user.email || "");
  const [isSavingProfile, setIsSavingProfile] = useState(false);

  // Armazenamento Real e Mídia
  const [offlineUsage, setOfflineUsage] = useState("Calculando...");
  const [showStorageSheet, setShowStorageSheet] = useState(false);
  const [storageMode, setStorageMode] = useState<"global" | "individual">(
    "global",
  );
  const [globalDays, setGlobalDays] = useState(90);

  // Calcula o armazenamento offline real usado pelo navegador/app
  useEffect(() => {
    if (navigator.storage && navigator.storage.estimate) {
      navigator.storage
        .estimate()
        .then((estimate) => {
          if (estimate.usage !== undefined) {
            const usageMB = (estimate.usage / (1024 * 1024)).toFixed(2);
            setOfflineUsage(`${usageMB} MB`);
          } else {
            setOfflineUsage("Desconhecido");
          }
        })
        .catch(() => setOfflineUsage("Erro ao ler"));
    } else {
      setOfflineUsage("Não suportado");
    }
  }, []);

  const sections: SettingSection[] = [
    {
      title: "Conta",
      items: [
        {
          icon: "user",
          label: "Perfil",
          value: user.name || "Usuário",
          action: () => setShowEditProfile(true),
        },
        {
          icon: "envelope",
          label: "E-mail",
          value: user.email || "Sem e-mail",
          action: () => setShowEditProfile(true),
        },
        { icon: "id-card", label: "Função", value: user.role || "Membro" }, // Sem action, não edita
      ],
    },
    {
      title: "Sistema & Dados",
      items: [
        {
          icon: "cloud",
          label: "Armazenamento e Mídia",
          value: "90 dias",
          action: () => setShowStorageSheet(true),
        },
        {
          icon: "cloud-download",
          label: "Dados Offline",
          value: offlineUsage, // Mostra os MBs reais!
        },
        {
          icon: "trash",
          label: "Limpar Cache",
          action: () => setShowClearCacheSheet(true),
          danger: true,
        },
      ],
    },
    {
      title: "Suporte",
      items: [
        { icon: "help-circle", label: "Central de Ajuda" }, // Sem action (estático)
        { icon: "bug", label: "Reportar Problema" }, // Sem action (estático)
        { icon: "info-circle", label: "Versão", value: "1.0.0" },
      ],
    },
  ];

  const retentionOptions = [
    { value: 30, label: "30 dias" },
    { value: 60, label: "60 dias" },
    { value: 90, label: "90 dias" },
    { value: 180, label: "6 meses" },
  ];

  const handleLogout = async () => {
    await signOut({ callbackUrl: "/login" });
  };

  const handleSaveProfile = async () => {
    setIsSavingProfile(true);
    try {
      const res = await fetch("/api/user", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: editName, email: editEmail }),
      });

      if (!res.ok) throw new Error("Falha ao atualizar");

      setShowEditProfile(false);
      router.refresh(); // Atualiza a página para pegar os novos dados do servidor
    } catch (error) {
      alert("Erro ao salvar o perfil. Tente novamente.");
    } finally {
      setIsSavingProfile(false);
    }
  };

  const handleClearCache = async () => {
    try {
      // Limpa Local e Session Storage
      localStorage.clear();
      sessionStorage.clear();

      // Limpa a API de Cache do Navegador (Service Workers/Arquivos Offline)
      if ("caches" in window) {
        const cacheNames = await caches.keys();
        await Promise.all(cacheNames.map((name) => caches.delete(name)));
      }

      alert("Cache limpo com sucesso! O aplicativo será reiniciado.");
      window.location.reload();
    } catch (error) {
      alert("Erro ao limpar cache.");
    }
  };

  return (
    <div className="flex-1 flex flex-col h-full bg-background relative">
      <header className="pt-safe sticky top-0 bg-background border-b border-border z-10">
        <div className="px-6 py-4 flex items-center justify-between">
          <button
            type="button"
            onClick={() => router.back()}
            className="w-10 h-10 rounded-md border border-input bg-transparent flex items-center justify-center active:bg-secondary/50 transition-colors"
          >
            <BoxIcon name="chevron-left" size={24} />
          </button>
          <div className="text-center">
            <h1 className="text-base font-bold text-foreground">
              Configurações
            </h1>
          </div>
          <NetworkStatusIndicator showLabel={false} />
        </div>
      </header>

      <main className="flex-1 px-6 pt-6 pb-24 lg:pb-8 overflow-y-auto space-y-8">
        <div className="p-4 rounded-md border border-border bg-transparent flex items-center gap-4">
          <div className="w-12 h-12 rounded-md bg-secondary/30 flex items-center justify-center">
            <BoxIcon name="user" size={24} className="text-muted-foreground" />
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-semibold text-foreground truncate">
              {user.name}
            </h2>
            <p className="text-sm text-muted-foreground truncate">
              {user.role}
            </p>
          </div>
          <button
            onClick={() => setShowEditProfile(true)}
            className="p-2 text-muted-foreground hover:text-foreground active:scale-95 transition-transform"
          >
            <BoxIcon name="pencil" size={20} />
          </button>
        </div>

        <div className="space-y-6">
          {sections.map((section) => (
            <section key={section.title}>
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
                {section.title}
              </h3>
              <div className="rounded-md border border-border bg-transparent divide-y divide-border">
                {section.items.map((item) => (
                  <button
                    key={item.label}
                    onClick={item.action}
                    disabled={!item.action}
                    className={cn(
                      "w-full flex items-center justify-between px-4 py-3 bg-transparent transition-colors text-left",
                      item.action &&
                        "active:bg-secondary/50 hover:bg-accent/50",
                      !item.action && "cursor-default",
                    )}
                  >
                    <div className="flex items-center gap-3">
                      {item.icon && (
                        <BoxIcon
                          name={item.icon as any}
                          size={20}
                          className={
                            item.danger
                              ? "text-destructive"
                              : "text-muted-foreground"
                          }
                        />
                      )}
                      <span
                        className={cn(
                          "text-sm font-medium",
                          item.danger ? "text-destructive" : "text-foreground",
                        )}
                      >
                        {item.label}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {item.value && (
                        <span className="text-sm text-muted-foreground">
                          {item.value}
                        </span>
                      )}
                      {item.action && !item.danger && (
                        <BoxIcon
                          name="chevron-right"
                          size={20}
                          className="text-muted-foreground/50"
                        />
                      )}
                    </div>
                  </button>
                ))}
              </div>
            </section>
          ))}
        </div>

        <button
          onClick={() => setShowLogoutSheet(true)}
          className="w-full h-12 rounded-md border border-destructive/30 bg-transparent flex items-center justify-center gap-2 active:bg-destructive/10 transition-colors"
        >
          <BoxIcon name="log-out" size={20} className="text-destructive" />
          <span className="font-medium text-sm text-destructive">
            Sair da Conta
          </span>
        </button>
      </main>

      {/* ==================== BOTTOM SHEET: EDITAR PERFIL ==================== */}
      <BottomSheet
        open={showEditProfile}
        onClose={() => !isSavingProfile && setShowEditProfile(false)}
        title="Editar Perfil"
      >
        <div className="px-4 pb-6 space-y-4">
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">
              Nome Completo
            </label>
            <input
              type="text"
              value={editName}
              onChange={(e) => setEditName(e.target.value)}
              className="w-full h-12 px-4 rounded-xl border border-input bg-transparent text-foreground outline-none focus:border-primary transition-colors"
            />
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">
              E-mail
            </label>
            <input
              type="email"
              value={editEmail}
              onChange={(e) => setEditEmail(e.target.value)}
              className="w-full h-12 px-4 rounded-xl border border-input bg-transparent text-foreground outline-none focus:border-primary transition-colors"
            />
          </div>
          <div className="pt-2 flex gap-3">
            <button
              onClick={() => setShowEditProfile(false)}
              disabled={isSavingProfile}
              className="flex-1 h-12 rounded-xl bg-secondary text-secondary-foreground font-medium active:scale-[0.98] disabled:opacity-50"
            >
              Cancelar
            </button>
            <button
              onClick={handleSaveProfile}
              disabled={isSavingProfile || !editName || !editEmail}
              className="flex-1 h-12 rounded-xl bg-primary text-primary-foreground font-bold active:scale-[0.98] disabled:opacity-50"
            >
              {isSavingProfile ? "Salvando..." : "Salvar"}
            </button>
          </div>
        </div>
      </BottomSheet>

      {/* ==================== BOTTOM SHEET: RETENÇÃO DE MÍDIA ==================== */}
      <BottomSheet
        open={showStorageSheet}
        onClose={() => setShowStorageSheet(false)}
        title="Limpeza de Mídia"
      >
        <div className="px-4 pb-6 space-y-6">
          <p className="text-sm text-muted-foreground text-center">
            Defina em quanto tempo as fotos e vídeos serão apagados
            automaticamente da nuvem para economizar espaço.
          </p>

          <div className="flex p-1 bg-secondary/30 rounded-lg">
            <button
              onClick={() => setStorageMode("global")}
              className={cn(
                "flex-1 py-2 text-sm font-medium rounded-md transition-all",
                storageMode === "global"
                  ? "bg-background shadow-sm text-foreground"
                  : "text-muted-foreground",
              )}
            >
              Regra Global
            </button>
            <button
              onClick={() => setStorageMode("individual")}
              className={cn(
                "flex-1 py-2 text-sm font-medium rounded-md transition-all",
                storageMode === "individual"
                  ? "bg-background shadow-sm text-foreground"
                  : "text-muted-foreground",
              )}
            >
              Por Obra
            </button>
          </div>

          {storageMode === "global" && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="grid grid-cols-2 gap-3">
                {retentionOptions.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => setGlobalDays(opt.value)}
                    className={cn(
                      "h-12 rounded-md border text-sm font-medium transition-all active:scale-95",
                      globalDays === opt.value
                        ? "border-primary bg-primary/10 text-primary"
                        : "border-border bg-transparent text-foreground",
                    )}
                  >
                    {opt.label}
                  </button>
                ))}
              </div>
              <button
                onClick={() => {
                  localStorage.setItem(
                    "@rdo:globalRetention",
                    globalDays.toString(),
                  );
                  alert(`Regra salva! Mídias expiram em ${globalDays} dias.`);
                  setShowStorageSheet(false);
                }}
                className="w-full h-12 mt-4 rounded-md bg-primary text-primary-foreground font-bold active:scale-[0.98] transition-transform"
              >
                Aplicar a Todas as Obras
              </button>
            </div>
          )}

          {storageMode === "individual" && (
            <div className="space-y-3 animate-in fade-in slide-in-from-left-4 duration-300">
              {projects.length === 0 && (
                <p className="text-sm text-center text-muted-foreground py-4">
                  Nenhuma obra cadastrada.
                </p>
              )}
              {projects.map((project) => (
                <div
                  key={project.id}
                  className="flex items-center justify-between p-3 rounded-md border border-border"
                >
                  <div className="flex items-center gap-3">
                    <BoxIcon
                      name="building"
                      size={20}
                      className="text-muted-foreground"
                    />
                    <span className="text-sm font-medium">{project.name}</span>
                  </div>
                  <select
                    className="bg-secondary/30 border-none text-sm font-medium rounded-md px-2 py-1 outline-none focus:ring-1 focus:ring-primary"
                    defaultValue="90"
                  >
                    <option value="30">30 dias</option>
                    <option value="60">60 dias</option>
                    <option value="90">90 dias</option>
                    <option value="180">6 meses</option>
                  </select>
                </div>
              ))}
              <button
                onClick={() => {
                  alert("Regras específicas salvas com sucesso!");
                  setShowStorageSheet(false);
                }}
                className="w-full h-12 mt-4 rounded-md bg-primary text-primary-foreground font-bold active:scale-[0.98] transition-transform"
              >
                Salvar Regras
              </button>
            </div>
          )}
        </div>
      </BottomSheet>

      {/* Logout */}
      <BottomSheet
        open={showLogoutSheet}
        onClose={() => setShowLogoutSheet(false)}
        title="Sair da Conta"
      >
        <div className="space-y-6 pb-6 px-4">
          <p className="text-sm text-muted-foreground text-center">
            Tem certeza que deseja sair?
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => setShowLogoutSheet(false)}
              className="flex-1 h-12 rounded-md border border-input bg-transparent text-sm font-medium active:bg-secondary/50"
            >
              Cancelar
            </button>
            <button
              onClick={handleLogout}
              className="flex-1 h-12 rounded-md bg-destructive text-destructive-foreground text-sm font-medium active:scale-[0.98]"
            >
              Sair
            </button>
          </div>
        </div>
      </BottomSheet>

      {/* Limpar Cache Real */}
      <BottomSheet
        open={showClearCacheSheet}
        onClose={() => setShowClearCacheSheet(false)}
        title="Limpar Cache"
      >
        <div className="space-y-6 pb-6 px-4">
          <p className="text-sm text-muted-foreground text-center">
            Isso removerá todos os dados offline do seu aparelho. Você precisará
            de internet para carregá-los novamente.
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => setShowClearCacheSheet(false)}
              className="flex-1 h-12 rounded-md border border-input bg-transparent text-sm font-medium active:bg-secondary/50"
            >
              Cancelar
            </button>
            <button
              onClick={handleClearCache}
              className="flex-1 h-12 rounded-md bg-destructive text-destructive-foreground text-sm font-medium active:scale-[0.98]"
            >
              Limpar Dados
            </button>
          </div>
        </div>
      </BottomSheet>
    </div>
  );
}
